...
`assign_interrupts *interrupt name* *hexadecimal cpu mask*`
...
Ideally, you would also use something like irqbalance to move all other interrupts away from the VM assigned CPU cores and onto your other hypervisor-reserved cores.
...
#assign_interrupts vfio 0000FCFC
...
```

In this particular use case, all interrupts with `vfio` are assigned to cores `0000FCFC`, which in term correspond to cores `2-7` and their SMT equivalents `10-15`.
